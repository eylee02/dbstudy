/*
    ...
    3. 종류
        ...
        2)  WHERE 절 : 단일 행 서브쿼리 (결과가 1개)
                       다중 행 서브쿼리 (결과가 N개)
*/
/*
    단일 행 서브쿼리(single low sub query)
    1. 결과가 1행이다. (1개이다)
    2. 단일 행 서브쿼리인 경우
...
-- 1. 사원번호가 101인 사원의 직업과 동일한 직업을 가진 사원을 조회하시오. (단일 행 서브쿼리)
SELECT *
  FROM EMPLOYEES
...
-- 2. 부서명이 'IT'인 부서에 근무하는 사원 조회하기 (다중 행 서브쿼리)
SELECT *
  FROM EMPLOYEES
...
SELECT HD, HIRE_DATE, EMPLOYEE_ID
  FROM (SELECT ROW_NUMBER() OVER(ORDER BY HIRE_DATE ASC) AS HD, HIRE_DATE, EMPLOYEE_ID
          FROM EMPLOYEES)
...
-- 부서번호가 50인 부서에서 근무하는 사원번호, 사원명, 부서명 조회하기(상관쿼리)

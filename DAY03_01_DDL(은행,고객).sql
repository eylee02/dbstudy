/*
    ...
    2.데이터베이스 객체를 다루는 언어
    3.트랜잭션 대상이 아니다.(작업(실행)을 취소할 수 없다.)
    4.종류 
        1)CREATE   : 생성
        2)ALTER    : 수정
        3)DROP     : 삭제
...
/*
    ...
    4.칼럼의 이름을 수정 : ALTER TABLE 테이블명 RENAME COLUMN 기존칼럼명 TO 신규칼럼명
    5.테이블 이름을 수정 : ALTER TABLE 테이블명 RENAME TO 신규테이블명
    6.PK/FK 제약조건
        1)PK
            1-추가  
        ...
        2)FK
            1-추가
                ALTER TABLE 자식테이블명 ADD CONSTRAINT 제약조건명 FOREIGN KEY(칼럼명) REFEREANCES 부모테이블명(참조할칼럼)
            2-삭제
                ALTER TABLE 테이블명 DROP CONSTRAINT 제약조건명
*/ 
...
--5.고객 테이블에 GRADE 칼럼을 추가하시오('VIP', 'GOLD', 'SILVER', 'BRONZE') 중 하나의 값을 가지도록 한다.
    ALTER TABLE CUSTOMER_TBL ADD GRADE VARCHAR2(15 BYTE) CHECK(GRADE = 'VIP' OR GRADE = 'GOLD' OR GRADE = 'SILVER' OR GRADE = 'BRONZE');
 -- ALTER TABLE CUSTOMER_TBL ADD GRADE VARCHAR2 (15 BYTE) CHECK(GRADE IN('VIP', 'GOLD', 'SILVER', 'BRONZE'));  

--6.고객 테이블에 고객명(NAME)과 연락처(PHONE) 칼럼 이름을 CUST_NAME, CUST_PHONE으로 변경하시오
    ALTER TABLE CUSTOMER_TBL RENAME COLUMN NAME TO CUST_NAME;
    ALTER TABLE CUSTOMER_TBL RENAME COLUMN PHONE TO CUST_PHONE;

--7.고객 테이블에 연락처(CUST_PHONE) 칼럼을 필수 칼럼으로 변경하시오
    ALTER TABLE CUSTOMER_TBL MODIFY CUST_PHONE VARCHAR2(30 BYTE) NOT NULL;

--8.고객 테이블의 고객명(CUST_NAME) 칼럼의 필수 제약조건을 없애시오.
    ALTER TABLE CUSTOMER_TBL MODIFY CUST_NAME VARCHAR2(30 BYTE) NULL; --필수제약 조건 변경시 NULL 값을 명시해야함
    
--테이블 구조 확인하기
    DESCRIBE BANK_TBL; --DESC로도 실행가능
    DESC CUSTOMER_TBL;

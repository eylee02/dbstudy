...
SELECT *
 ...
 ORDER BY SALARY DESC;         -- 구분기호못넣음

-- 13. EMPLOYEES 테이블에서 전체 사원의 근무 개월 수를 정수로 조회하시오. 1개월 1일을 근무했다면 2개월을 근무한 것으로 처리해서 조회하시오.
SELECT CEIL(MONTHS_BETWEEN(SYSDATE,TO_DATE(HIRE_DATE , 'YY/MM/DD'))) AS 근무개월수
  FROM EMPLOYEES;
...
SELECT PHONE_NUMBER
     , SUBSTR(PHONE_NUMBER,1,3) AS 지역번호
...
SELECT EMPLOYEE_ID, FIRST_NAME, LAST_NAME, EMAIL, PHONE_NUMBER, HIRE_DATE, JOB_ID
     , SALARY, COMMISSION_PCT, MANAGER_ID, DEPARTMENT_ID, RANK() OVER(PARTITION BY DEPARTMENT_ID ORDER BY SALARY) AS 부서내연봉순위
  FROM EMPLOYEES;
...
SELECT SUBSTR(DEPARTMENT_NAME, 1, 2) ,COUNT(*) AS 부서수
  FROM DEPARTMENTS

...
-- START WITH 1 INCREMENT BY 1 1부터시작해서 1씩증가한다
-- NOMAXVALUE NOMINVALUE  최대값,최소값없음
-- NOCYCLE CACHE 20 ORDER 반복없다 캐시는20개 순서대로
...
-- 가격의순위구하기 (인라인뷰 사용 FROM절에 SELECT문먼저사용)
SELECT *
...
SELECT O.ORDER_ID AS 주문번호
 ...
 WHERE TO_DATE(O.ORDER_DT) BETWEEN TO_DATE('20/07/05') AND TO_DATE('20/07/09') -- INSERT INTO에서 TO_DATE로 입력했으면 칼럼에 TO_DATE를 안적어도됨 
 ORDER BY O.ORDER_DT;
...
SELECT O.ORDER_ID AS 구매번호
  ...
  FROM BOOK_T B INNER JOIN ORDER_T O
    ...
    ON C.CUST_ID = O.CUST_ID
 WHERE TO_DATE(O.ORDER_DT) NOT BETWEEN TO_DATE('20/07/04') AND TO_DATE('20/07/07');
...
-- 12. 가장 최근에 구매한 고객의 이름, 책이름, 주문일자를 조회하시오.(최근은 MAX)
SELECT C.CUST_NAME AS 고객명
     , B.BOOK_NAME AS 책이름
     , O.ORDER_DT AS 주문일자
  FROM BOOK_T B INNER JOIN ORDER_T O
    ON B.BOOK_ID = O.BOOK_ID INNER JOIN CUSTOMER_T C
    ON C.CUST_ID = O.CUST_ID
 WHERE TO_DATE(O.ORDER_DT) = (SELECT MAX(TO_DATE(ORDER_DT))
                                FROM ORDER_T);
...
-- '김연아'가 구매한 책이 몇 종인가?(피겨의 정석을 2번 주문했다면 구매한 책은 1종)
SELECT C.CUST_NAME AS 고객명
     , COUNT(DISTINCT O.BOOK_ID) AS 책종류 
   FROM CUSTOMER_T C INNER JOIN ORDER_T O
    ON C.CUST_ID = O.CUST_ID
 WHERE C.CUST_NAME = '김연아'
 GROUP BY C.CUST_ID, C.CUST_NAME;

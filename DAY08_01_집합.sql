...
SELECT 1,2
  FROM DUAL
 UNION ALL  -- 중복을 그대로 조회
SELECT 1,2
  FROM DUAL;
  
  
-- 사원테이블과 부서테이블에 존재하는 모든 부서번호를 조회하기
...
-- 사원 테이블과 부서 테이블에 모두 본재하는 부서번호 조회하기(교집합)
...
WITH MY_SUBQUERY(LVL, EMPLOYEE_ID, FIRST_NAME, LAST_NAME, MANAGER_ID) AS (
    ...
    SELECT 1, EMPLOYEE_ID, FIRST_NAME, LAST_NAME, MANAGER_ID
     ...
     UNION ALL
    -- 반복해서 호출되는 서브쿼리
    SELECT M.LVL +1 LVL, E.EMPLOYEE_ID, E.FIRST_NAME, E.LAST_NAME, E.MANAGER_ID
